...
| 땅따먹기 (선 그리기) | 14×14, 칸 사이 격자선 위로 한 획에 선을 그어 내 땅으로 되돌아오면 둘러싼 칸을 차지 (한 획 최대 16칸, 차례마다 제한 시간이 24초에서 6초까지 점점 짧아짐) |
| 땅따먹기 (돌 튕기기) | 연속 공간에서 돌을 새총처럼 당겨 치면 마찰로 미끄러지다 멈추고(거리는 세기의 제곱에 비례), 3번 안에 내 땅에 멈추면 지나온 영역 차지 |
...
- **shared**: 게임마다 `createInitialState / legalMoves / applyMove / status / pieces`
  ...
  단, 한 번의 제스처가 곧 한 수인 두 땅따먹기(선 하나 전체 / 돌을 튕기는 방향·세기)는
  합법수를 전부 나열할 수 없어 `legalMoves`가 표본만 돌려줍니다. `applyMove`는 규칙에
  맞는 수라면 표본에 없어도 전부 받아들이고, 클라이언트는 각 엔진이 따로 export한
  판정 함수(예: `territoryCanExtend`)로 그리는 중에도 같은 규칙을 씁니다.
- **server**: 계정/DB 없이 방 코드(6자리)로만 동작하는 실시간 대전 서버입니다. 방 상태는
  전부 메모리에 있고, 아무도 접속하지 않은 방은 주기적으로 정리됩니다. 새로고침해도
  `localStorage`에 저장된 좌석 토큰으로 같은 자리를 되찾습니다. 제한 시간이 있는 게임
  (`GameEngine.clock`)은 서버가 방마다 타이머를 돌려 시간이 다 되면 직접 시간초과 수를
  두고 알립니다 — 규칙은 엔진에, 시계는 서버에 있습니다. 둘이 다 접속해 있을 때만
  시계가 돌아가므로, 상대를 기다리거나 연결이 끊긴 동안에는 차례가 타지 않습니다.
