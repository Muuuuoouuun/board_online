import { TERRITORY_MAX_LINE, TERRITORY_TURN_FLOOR_MS, TERRITORY_TURN_START_MS } from "@board-online/shared";
...
export const territoryRules = {
  ...
  points: [
    ...
    `차례마다 제한 시간이 있고, 한 차례가 지날 때마다 조금씩 짧아집니다. 첫 차례는 ${Math.round(
      TERRITORY_TURN_START_MS / 1000,
    )}초이고 ${Math.round(
      TERRITORY_TURN_FLOOR_MS / 1000,
    )}초까지 줄어듭니다. 초반에는 느긋하게 궁리하다가 후반에는 손이 바빠지는 게임입니다.`,
    "시간이 다 되면 그리던 선은 사라지고 아무것도 차지하지 못한 채 차례가 넘어갑니다. 온라인에서는 상대가 들어오기 전이나 연결이 끊긴 동안에는 시계가 멈추고, 다시 이어지면 그 차례를 처음부터 다시 셉니다.",
